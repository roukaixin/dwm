...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		...
		resize(c, m->wx + gappov, m->wy + gappoh, m->ww - 2 * c->bw - 2 * gappov, m->wh - 2 * c->bw - 2 * gappoh, False);
}
...
updatesystray(void)
{
	...
	unsigned int sws = TEXTW(stext) - lrpad + systrayspacing;
	unsigned int w = 1;
